...
public abstract class AbstractCompanyGenerator {
    ...
    private void processFinances(final Campaign campaign,
        ...
        if (getOptions().isPayForSetup()) {
            ...
            final Money partCosts = options.isPayForParts() ? AbstractPartGenerator.calculatePartCosts(parts) : Money.zero();
            final Money armourCosts = options.isPayForArmour() ? BasicArmourGenerator.calculateArmourCosts(armour) : Money.zero();
            final Money ammunitionCosts = options.isPayForAmmunition() ? BasicAmmunitionGenerator.calculateAmmunitionCosts(ammunition) : Money.zero();
